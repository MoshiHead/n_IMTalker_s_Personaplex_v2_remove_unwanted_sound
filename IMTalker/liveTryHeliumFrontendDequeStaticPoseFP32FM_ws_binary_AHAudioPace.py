...
class MoshiOnlyEngineWithHidden(MoshiOnlyEngine):
    ...
    def __init__(
        ...
        *args,
        ...
        prehold_mode: str = "stop",
        prehold_max_sec: float = 8.0,
        early_cue_silence_frames: int = 3,
        **kwargs,
    ) -> None:
        self.tf_capture_layer = int(capture_layer)

        # ── Pre-decision hold + early thinking cue ──────────────────────────
        #
        # Root cause of the "extra speech before the thinking sound" reported
        # against conversation_log_1 ("I don't have the..." before a gold-price
        # search, "Tesla's..." before a stock-price search):
        #
        #   PersonaPlex is FULL-DUPLEX. It starts composing its own reply within
        #   about one 80ms chunk of the user going quiet. The routing/search
        #   machinery, however, cannot even START until the STT VAD has seen
        #   _VAD_SILENCE_FRAMES_REQUIRED (12) frames of silence -- ~960ms. For
        #   that whole ~1s the model's text and audio go out unmuted, so the
        #   user hears the opening of an ungrounded, invented answer; only then
        #   does _start_turn() fire, mute the text stream and start the thinking
        #   sound. That leaked fragment also never reached either log, because
        #   _turn_start_audio_text_len was snapshotted at _start_turn -- i.e.
        #   AFTER the leak had already been appended to self.audio_text -- so it
        #   fell into the dead zone between two turns' slice bounds.
        #
        # The fix has two halves:
        #
        #   1. PRE-DECISION HOLD (`prehold_mode`). Force the model's text stream
        #      to its own zero_text_code from the moment the user stops speaking
        #      until the route is known. The hold is released the instant the
        #      decision is "no search" (so a knowledge answer starts essentially
        #      immediately), or handed over to the existing search-window hold
        #      (`suppress_text_until_ref`) when a search is committed.
        #
        #   2. EARLY THINKING CUE (`early_cue_silence_frames`). Run the pure
        #      regex router on the PARTIAL transcript a few frames into the
        #      silence, instead of waiting the full ~960ms VAD window. When it
        #      already says "search", the thinking sound starts right there, so
        #      the user hears the cue ~240ms after finishing rather than ~1s
        #      later, with nothing spoken in between.
        #
        # Both are gated on `search_enabled`; with search off, nothing here ever
        # engages and the plain conversational path is untouched.
        mode = str(prehold_mode or "stop").strip().lower()
        if mode not in ("off", "stop", "start"):
            print(
                f"[liveTryPlasticity][search] unknown prehold_mode {prehold_mode!r} "
                f"-- falling back to 'stop'",
                flush=True,
            )
            mode = "stop"
        self.prehold_mode = mode
        # Safety valve only. The hold is normally released within ~1s by the
        # routing decision; this cap exists so that a dropped decision (a killed
        # background thread, a rejected transcript on an unusual path) can never
        # mute the avatar for the rest of the session.
        self._PRE_DECISION_HOLD_MAX_FRAMES = max(
            1, round(float(prehold_max_sec) * TARGET_SR / MIMI_FRAME_SIZE)
        )
        # 0 disables the early cue entirely (the sound then starts at _start_turn
        # as before). 3 frames = ~240ms, chosen to be long enough that a natural
        # mid-sentence pause rarely reaches it, short enough to feel immediate.
        self._EARLY_CUE_SILENCE_FRAMES = max(0, int(early_cue_silence_frames))
    ...
    # ── Pre-decision hold ───────────────────────────────────────────────────
    #
    # Everything below runs on the GPU thread only (from _stt_step / _step /
    # _consume_pending), so no locking is needed -- same discipline as the
    # thinking-sound state above.

    def _start_pre_decision_hold(self, why: str) -> None:
        """Mute the model's text stream until the route for this utterance is
        known. Idempotent and latched: once engaged for an utterance it stays
        engaged until an explicit release, so a momentary VAD flicker cannot
        un-mute the model mid-question."""
        if self.pre_decision_hold or self.prehold_mode == "off":
            return
        if not getattr(self, "search_enabled", False) or self.search_hard_disabled:
            return
        self.pre_decision_hold = True
        self._pre_hold_frames = 0
        # Where this turn's response begins. Snapshotted HERE, at the moment the
        # hold engages, rather than at _start_turn ~1s later: anything the model
        # still manages to emit before the hold takes effect then lands inside
        # this turn's logged response instead of vanishing between turns.
        self._pre_hold_audio_text_len = len(self.audio_text)
        self.conv_logger.event(
            "prehold_start", reason=why, mode=self.prehold_mode,
        )

    def _release_pre_decision_hold(self, why: str) -> None:
        """Let the model speak again. Safe to call unconditionally."""
        if not self.pre_decision_hold:
            return
        self.pre_decision_hold = False
        held_s = self._pre_hold_frames * MIMI_FRAME_SIZE / TARGET_SR
        self._pre_hold_frames = 0
        self.conv_logger.event(
            "prehold_release", reason=why, held_s=round(held_s, 2),
        )

    def _turn_response_start_index(self) -> int:
        """Index into self.audio_text where this turn's response begins.

        Prefers the pre-hold snapshot (taken when the user stopped speaking)
        over `len(self.audio_text)` now, so the ~1s routing window is attributed
        to this turn rather than falling between two turns' slice bounds and
        disappearing from the logs entirely."""
        snapshot = self._pre_hold_audio_text_len
        if 0 <= snapshot <= len(self.audio_text) and snapshot >= self._utterance_start_audio_text_len:
            return snapshot
        return len(self.audio_text)

    def _cancel_early_thinking_cue(self, reason: str, reason_text: str) -> None:
        """Undo a speculative early cue that never became a committed turn.

        Only meaningful before `search_awaiting_ref` is set: once a turn is
        committed, _consume_pending owns the clip and its two real endings
        (<ref> injected, or filler timeout). The `suppress_text_until_ref` reset
        is the part that matters -- _start_thinking_sound raises that flag as a
        side effect, and with no committed turn there is no _consume_pending
        running to ever lower it again."""
        if self.search_awaiting_ref:
            return
        if not self.search_thinking_active and not self.suppress_text_until_ref:
            return
        self.suppress_text_until_ref = False
        self._stop_thinking_sound(self.search_turn_epoch + 1, reason, reason_text)

    def _maybe_early_thinking_cue(self) -> None:
        """Start the thinking sound BEFORE the VAD has confirmed end-of-turn,
        when the partial transcript already rule-routes to 'search'.

        Only the pure-regex router is used here -- microseconds, no forward
        pass, and deterministic, so this is safe on _step()'s 80ms budget. A
        partial transcript is enough for the shapes that matter: 'what is
        today's gold price' carries its time word and its 'price' long before
        the sentence ends.

        Costs nothing when wrong: if the completed transcript turns out not to
        need a search, _begin_casual_turn (rules) or _consume_pending's cancel
        branch (model router) stops the clip again."""
        if self._early_cue_checked or self.search_thinking_active:
            return
        if self._EARLY_CUE_SILENCE_FRAMES <= 0:
            return
        if self.stt_silence_frame_count < self._EARLY_CUE_SILENCE_FRAMES:
            return
        # One attempt per utterance, whatever the verdict.
        self._early_cue_checked = True
        if not getattr(self, "search_enabled", False) or not self._will_actually_search():
            return

        import search_helpers

        partial = search_helpers.decode_stt_tokens(
            self.stt_token_buffer, self.stt_tokenizer, self.stt_padding_token_id
        )
        if not partial.strip():
            return
        # The same sanity gate _stt_step applies to the final transcript. A
        # garbage or non-English partial must not trigger an audible cue for a
        # turn that is about to be thrown away.
        usable, _stats = search_helpers.check_transcript_usable(
            partial, self.stt_max_non_latin_ratio, require_english=self.stt_require_english,
        )
        if not usable and self.stt_reject_foreign_script:
            return
        ruled, rule_reason = search_helpers.rule_route_explain(partial)
        if ruled is not True:
            return
        self.conv_logger.event(
            "thinking_sound_early_cue", transcript=partial, reason=rule_reason,
            silence_frames=int(self.stt_silence_frame_count),
        )
        print(
            f"[liveTryPlasticity][search] early thinking cue after "
            f"{self.stt_silence_frame_count} silent frames ({rule_reason}) :: {partial!r}",
            flush=True,
        )
        self._start_thinking_sound(self.search_turn_epoch + 1, partial)
    ...
    def reset_session(self) -> None:
        ...
        self.suppress_text_until_ref = False
        # Pre-decision hold: the model is muted from the moment the user stops
        # speaking until the route for that utterance is known. This is what
        # closes the ~1s full-duplex window between end-of-question and
        # _start_turn, in which the model used to speak the opening of an
        # ungrounded answer ("I don't have the...", "Tesla's...") before the
        # thinking sound could start. See _start_pre_decision_hold.
        self.pre_decision_hold = False
        self._pre_hold_frames = 0
        # -1 = "no snapshot taken for this turn"; _turn_response_start_index
        # then falls back to len(self.audio_text), the old behavior.
        self._pre_hold_audio_text_len = -1
        # One early-cue attempt per utterance (see _maybe_early_thinking_cue),
        # plus its own watchdog counter (a cue can outlive the hold, and can be
        # raised with the hold disabled entirely).
        self._early_cue_checked = False
        self._early_cue_frames = 0
    ...
    def _stt_step(self, chunk: torch.Tensor) -> None:
        ...
        # Pre-decision hold watchdog. Ticked here because _stt_step runs on
        # every single chunk, whereas _consume_pending only runs once a turn is
        # already in flight -- and the hold exists precisely to cover the window
        # BEFORE that. Without this, a decision that never arrives (killed
        # thread, an exit path that forgot to release) would leave the avatar
        # mute for the rest of the session.
        if self.pre_decision_hold:
            self._pre_hold_frames += 1
            if self._pre_hold_frames >= self._PRE_DECISION_HOLD_MAX_FRAMES:
                held_s = self._pre_hold_frames * MIMI_FRAME_SIZE / TARGET_SR
                print(
                    f"[liveTryPlasticity][search] pre-decision hold hit its "
                    f"{held_s:.1f}s cap with no routing decision -- releasing the "
                    f"model so it can speak",
                    flush=True,
                )
                self._release_pre_decision_hold("hold timed out before any routing decision")

        # Second watchdog, for the early cue specifically. It needs its own
        # counter rather than riding on the one above, because the cue can be
        # raised with --search_prehold_mode off, where there is no hold to tick.
        # An early cue lives outside _consume_pending (which only runs once
        # search_awaiting_ref is set), so without this a cue on an utterance
        # that never commits to a turn would loop for the rest of the session.
        if self.search_thinking_active and not self.search_awaiting_ref:
            self._early_cue_frames += 1
            if self._early_cue_frames >= self._PRE_DECISION_HOLD_MAX_FRAMES:
                print(
                    "[liveTryPlasticity][search] early thinking cue outlived its cap "
                    "with no turn committed -- stopping it",
                    flush=True,
                )
                self._cancel_early_thinking_cue(
                    "early_cue_timeout", "no turn was ever committed for this utterance",
                )
        else:
            self._early_cue_frames = 0
        ...
        if stt_tokens is not None:
            ...
            if text_token not in (0, self.stt_padding_token_id):
                if not self.stt_in_utterance:
                    ...
                    self._pre_hold_audio_text_len = -1
                    self._early_cue_checked = False
                    # prehold_mode="start" mutes for the whole question, not
                    # just the gap after it. Only needed if the VAD's silence
                    # head lags enough to let a syllable through in "stop"
                    # mode; costs a longer run of forced zero-text, which biases
                    # the model towards staying quiet, hence not the default.
                    if self.prehold_mode == "start" and not self.search_awaiting_ref:
                        self._start_pre_decision_hold("user started speaking")
                self.stt_in_utterance = True
                self.stt_last_vad_end = False
        if vad_score > self.vad_threshold:
            self.stt_silence_frame_count += 1
            # THE fix for the leaked fragment. The model reacts to the finished
            # question within roughly one chunk, but _start_turn is ~960ms away
            # (_VAD_SILENCE_FRAMES_REQUIRED). Muting on the very first silent
            # frame closes that window to ~80ms -- too little for a word.
            if (
                self.prehold_mode == "stop"
                and self.stt_in_utterance
                and not self.search_awaiting_ref
            ):
                self._start_pre_decision_hold("user stopped speaking; awaiting routing decision")
            if self.stt_in_utterance and not self.search_awaiting_ref:
                self._maybe_early_thinking_cue()
        else:
            # Non-silent frame. Between the early cue (~3 silent frames) and the
            # VAD's end-of-turn (12), silence is monotonic, so a reset here means
            # the user carried on talking and the pause was mid-sentence. Drop
            # the speculative cue rather than play it over them; the turn falls
            # back to starting the sound at _start_turn, and the pre-decision
            # hold -- which is latched, not cancelled -- still keeps the model
            # from speaking over the rest of the question.
            if self.stt_silence_frame_count > 0 and not self.search_awaiting_ref:
                self._cancel_early_thinking_cue(
                    "user_resumed", "the user kept speaking, so the pause was not the end of the question",
                )
            self.stt_silence_frame_count = 0
        vad_fired = (
        ...
        if vad_fired and self.stt_in_utterance and self.stt_token_buffer and not self.search_awaiting_ref:
            ...
            if not transcript.strip():
                # VAD fired but nothing was transcribed (breath, noise, a cough).
                # No turn starts, so release the hold here -- otherwise the model
                # stays muted until the watchdog fires seconds later.
                self._release_pre_decision_hold("no transcript; not a turn")
                self._cancel_early_thinking_cue(
                    "empty_transcript",
                    "nothing was actually transcribed, so nothing was searched",
                )
            if transcript.strip():
                ...
                if not usable and self.stt_reject_foreign_script:
                    ...
                    # This turn is being thrown away, so nothing will ever
                    # decide a route for it. Undo both halves of the
                    # pre-decision state or the avatar stays mute (and, if the
                    # partial transcript happened to look like a live query,
                    # keeps playing a thinking sound) for no reason.
                    self._release_pre_decision_hold("transcript rejected before routing")
                    self._cancel_early_thinking_cue(
                        "transcript_rejected",
                        "the transcript was unusable, so nothing was searched",
                    )
                    return
    ...
    def _begin_casual_turn(self, transcript: str, reason: str) -> None:
        ...
        self._turn_start_audio_text_len = self._turn_response_start_index()
        ...
        #
        # This is the "no search needed" branch of the required flow:
        #   user question -> decide no lookup -> answer immediately.
        # Both holds come off right here, before returning, so the very next
        # generation step is free to speak. The release is the LAST thing the
        # decision does, so the added latency is one rule-router pass
        # (microseconds) rather than the ~1s the VAD window used to cost.
        ...
        self.suppress_text_until_ref = False
        self._release_pre_decision_hold(f"no search needed ({reason})")
    ...
    def _start_turn(self, transcript: str) -> None:
        ...
        # Hand the pre-decision hold over to the search-window hold. Both force
        # the same zero_text_code, so this is a bookkeeping change only -- the
        # model stays continuously muted from the moment the user stopped
        # speaking right through to the <ref> injection, with no gap between the
        # two mechanisms for a fragment to escape through.
        #
        # `suppress_text_during_search` is honored here exactly as it is in
        # _start_thinking_sound: with it off, the pre-decision hold is released
        # and the old behavior (audio muted, text still composed) is restored.
        if self.pre_decision_hold:
            if self.suppress_text_during_search:
                self.suppress_text_until_ref = True
                self._release_pre_decision_hold("handed over to the search-window hold")
            else:
                self._release_pre_decision_hold("suppress_text_during_search is off")
    ...
    def _consume_pending(self) -> None:
        ...
        if self.pending_search_cancelled:
            ...
            self.pending_start_thinking = False
            self.suppress_text_until_ref = False
            # Defensive: _start_turn normally hands the pre-decision hold over
            # to suppress_text_until_ref before this can run, so there is
            # usually nothing left to release. It matters on the
            # --suppress_text_during_search 0 path, where no handover happens.
            self._release_pre_decision_hold("router decided no search was needed")
            self.search_awaiting_ref = False
        ...
        if self.pending_ref_tokens is not None:
            ...
            # generation step must be free to speak the grounded answer. Both
            # holds are dropped -- the pre-decision one should already be gone
            # by now, but leaving it set would silently mute the grounded reply.
            self.suppress_text_until_ref = False
            self._release_pre_decision_hold("grounded answer ready")
            ...
            self.search_ref_committed_this_turn = True
            self.suppress_text_until_ref = False
            self._release_pre_decision_hold("search timed out")
            self._stop_thinking_sound(
    ...
    def _step(self, pcm24: np.ndarray) -> dict:
        ...
        # While a search is in flight -- or while the route for the utterance
        # that just ended is still being decided -- hand the model its own "say
        # nothing" token instead of letting it sample text.
        # process_transformer_output honors a provided token rather than
        # sampling, so this is the model's native silence mechanism (the same
        # one _step_audio_silence_core uses) -- not a hack bolted on top. Audio
        # and hidden states keep flowing, so the avatar pipeline and chunk
        # cadence are untouched; only the words are withheld, and they are
        # withheld precisely while the model would otherwise be inventing an
        # answer it is about to be handed.
        #
        # The two flags cover two adjacent windows and together leave no gap:
        #   pre_decision_hold      user stopped speaking -> route decided
        #   suppress_text_until_ref route decided (search) -> <ref> injected
        t_lm0 = time.perf_counter()
        if (self.suppress_text_until_ref or self.pre_decision_hold) and self._step_supports_text_token:
...
class LiveHeliumFMOptions(BaseOptions):
    def initialize(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--search_prehold_mode", default="stop", choices=("off", "stop", "start"),
            help="When to mute the model's text stream while the route for an utterance is still "
                 "being decided. PersonaPlex is full-duplex and begins its reply within ~1 chunk of "
                 "the user going quiet, but routing cannot start until the STT VAD has seen ~960ms "
                 "of silence -- so without this the model speaks the opening of an ungrounded answer "
                 "before the thinking sound can start ('I don't have the...' ahead of a gold-price "
                 "search, 'Tesla's...' ahead of a stock-price search; both were audible yet absent "
                 "from the logs, because the leak landed between two turns' slice bounds). "
                 "'stop' (default) mutes from the first silent frame, closing that window to ~80ms. "
                 "'start' mutes from the user's first transcribed word -- use only if a syllable "
                 "still escapes, since longer runs of forced silence bias the model towards staying "
                 "quiet. 'off' restores the old behavior.",
        )
        parser.add_argument(
            "--search_prehold_max_sec", type=float, default=8.0,
            help="Safety cap on the pre-decision hold. Normally released within ~1s by the routing "
                 "decision; this only matters if a decision never arrives (killed background thread, "
                 "an unusual exit path), where it stops the avatar being muted for the rest of the "
                 "session. Raise it only if you also raise --search_max_filler_sec well past it.",
        )
        parser.add_argument(
            "--search_early_cue_frames", type=int, default=3,
            help="Frames of silence (80ms each) after which the pure-regex router is run on the "
                 "PARTIAL transcript so the thinking sound can start immediately, instead of waiting "
                 "the full ~960ms VAD window. 3 (default) = ~240ms: long enough that a mid-sentence "
                 "pause rarely reaches it, short enough that the cue feels immediate. Only the regex "
                 "tier runs here (microseconds, deterministic, no forward pass), and only when web "
                 "search can actually be reached. A cue raised on a pause that turns out to be "
                 "mid-sentence is cancelled the moment the user speaks again. Set 0 to disable and "
                 "start the sound at end-of-turn as before.",
        )
        parser.add_argument(
    ...
    def get_moshi_engine() -> MoshiOnlyEngine:
        ...
        if moshi_engine is None:
            moshi_engine = MoshiOnlyEngineWithHidden(
                ...
                prehold_mode=str(getattr(args, "search_prehold_mode", "stop")),
                prehold_max_sec=float(getattr(args, "search_prehold_max_sec", 8.0)),
                early_cue_silence_frames=int(getattr(args, "search_early_cue_frames", 3)),
            )
        return moshi_engine
